...
### 2.1 Platform Layer (`tg_gemini.platform.telegram`)
...
- **Security:** Implements middleware-level user ID filtering based on the `allowed_user_ids` whitelist.

### 2.2 Core Layer (`tg_gemini.core.engine`)
...
### 2.3 Agent Layer (`tg_gemini.core.agent`)
- **Execution:** Uses `asyncio.create_subprocess_exec` to launch the Gemini CLI in a non-blocking way.
